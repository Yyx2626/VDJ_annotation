...
#	command = 'Yyx_check_col_num.pl ' + input_tlx_filename
#	if is_tlx_gzipped:
#		command = 'Yyx_check_col_num.pl <(zcat {} | head -n2)'.format(input_tlx_filename)
...
#	original_colnum = int(headline_colnum_str)
	...
	if not exist_file_or_dir(output_filenames[6], 'So skip intersectBed part...', mode='any'):
		stop_if_file_not_exist(f'{scripts_root_dirname}/yyx_tlx_seq_seg_and_bed_anno.20220429.pl')
		command = ('z' if is_tlx_gzipped else '') + 'cat "' + input_tlx_filename + f'" | perl {scripts_root_dirname}/yyx_tlx_seq_seg_and_bed_anno.20220429.pl {bed_filename} >' + output_filenames[6]
	...
	if not exist_file_or_dir(output_filenames[8], 'So skip miD_LCS part...', mode='any'):
		stop_if_file_not_exist('{}/yyx_annotate_tlx_midD_LCS.20220429.pl'.format(scripts_root_dirname))
	#	list_assign(output_filenames, 8, '{}.annotate_tlx_midD_LCS.{}.tlx'.format(output_prefix, date_postfix) )
		command = ('z' if is_tlx_gzipped else '') + 'cat "' + output_filenames[6] + f'" | perl {scripts_root_dirname}/yyx_annotate_tlx_midD_LCS.20220429.pl {D_ref_filename} 3 >' + output_filenames[8]
...
#	command = 'cat {} | python3 {}/yyx_show_or_skip_or_retrieve_columns.20190122.py skip '.format(output_filenames[8], scripts_root_dirname) + "'^count_in_file_.*'" + ' >' + output_filenames[15]
	command = 'cp ' + output_filenames[8] + ' ' + output_filenames[15]
